#!/usr/bin/env python3
"""
engine.py — Φ-I-C-K-P-7AXIS 교육 평가 엔진 (FINAL)

리뷰 95점 — 최종 마감:
  v1.0 (82점): axis_a 음항 상쇄 버그
  v1.1 (89점): axis_c 가중치 아티팩트
  v1.2 (96점): 4축 통일 + 판정식 + norm()
  FINAL (95점): norm 보정 설명 + 솔루션 효과 placeholder 고지

norm() 보정 규칙:
  norm(raw,min,max)으로 0~1 변환 후, 맥락 보정계수(0.85~1.00)를 곱해 최종값 도출.
  예: norm(27.5,0,30)=0.917 → 보정계수 0.98 적용 → 0.90
  보정계수는 Phase C에서 KEDI/OECD 패널 데이터로 대체 예정.
"""
